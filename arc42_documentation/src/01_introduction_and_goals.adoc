...
Das Smartphone ist zu einem digitalen Alltagsgegenstand mutiert, welches meist häufiger als ein traditioneller Desktop-PC oder Laptop verwendet wird. Diesbezüglich muss bei der Architektur einer aktuellen Webapplikation speziell darauf geachtet werden, welche Priorität den Aspekten __Responsiveness__ (Design und Verhalten der Applikation auf unterschiedlichen Geräten / Umgebungen) und __Progressive Web App__ (Erweiterung der Funktionalitäten gemäss Laufzeitumgebung, z.B. Push Notifications) zugeordnet werden soll.
