...
Mit den folgenden Taktiken soll die Einhaltung der Qualitätszenarien ermöglicht werden. Die Gewichtung der Szenarien wurde bei der Auswahl der Taktiken beachtet.
...
Request zur Ausführung einer Operation bezüglich einer TherapieSession gesendet
...
Falls eine dieser Überprüfungen Fehlschlägt, muss der Request geloggt werden und mit "400 bad request" beantwortet werden.

Requests dürfen nicht benötigte Daten mitliefern. Diese werden von den Request handlern jedoch rausgefiltert und ignoriert.
...
|*Stimulus*
|Request wird erhalten, für welchen die Kommunikation zwischen mehreren Systemen benötigt wird

|*Taktik*
|Wenn von einem Container von Physio Connect aus mit einem anderen internen Container oder einem externen System kommuniziert wird, sollen folgendermassen vorgegangen werden:
...
Im Falle eines Timeouts ein Retry Mechanismus verwendet mit mindestens drei Versuchen.

Im Falle eines Fehlers, bei welchem definiert ist, dass ein Retry Sinnvoll ist, ebenfalls ein Retry Mechanismus verwenden.
...
|*Taktik*
|Alle APIs in der Produktion sind nur via TLS / SSL

Kommunikation zwischen Containern ist in der Produktion immer verschlüsselt.
...
|*Stimulus*
|Request zum Auslesen von Messdaten wird erhalten
...
Es wird überprüft, ob der Benutzer die benötigten Sicherheitsrollen zugewiesen hat, um die entsprechenden Requests auszuführen. (z.B. Physiotherapeuten und Patientenrollen)

Es wird überprüft, ob der Benutzer die korrekte Identität hat, um entsprechende Requests auszuführen. (z.B. eigene Messdaten abrufen)
|===
...
|*Stimulus*
|Neuer API Endpoint implementiert werden

|*Taktik*
|Beim Implementieren von neuen API Requests muss das folgende beachtet werden

Ohne Begründung und Absprache ist kein Request ohne Authentifizierung erreichbar

Ohne Begründung und Absprache ist kein Request ohne Autorisierung durch mindestens eine Rolle erreichbar

Der Zugang zu allen Endpoints wird standardmässig verweigert und nur via Zugehörigkeit zu Rollen erlaubt. (Deny all)

Endpoints dürfen nicht für Benutzer offen sein, welche diesen Zugriff nicht benötigen. Falls zum Beispiel nur ein Teil der Physiotherapeuten einen Request verwendet, soll eine spezifische Rolle für dies Untergruppe erstellt werden.
...
Werde Domänenspezifische Anpassungen benötigt werden, muss das Domänenmodell angepasst / erweitert werden. Die darin enthaltene Logik soll der Realität entsprechen und somit auf zukünftige Änderungen reagieren können und möglichst unabhängig von Frameworks sein. Falls möglich werden die Änderungen mit einem Domänenexperten besprochen.

Bei allen Änderungen muss abgeschätzt werden, wie wahrscheinlich Änderungen in der Zukunft sind. Grundsätzlich soll nur in extremen Fällen hohe Abstraktionen eingesetzt werden, um Over Engineering zu vermeiden.

Bei der Umsetzung aller Funktionalitäten muss die Kohäsion und Korrelation beachtet werden. Refactorings, zum Beispiel zum Aufsplitten von Modulen, Neuverteilung von Verantwortungen oder sogar Auslagerung in einen neuen Microservices, sind Teil aller Stories.
|===
...
* Wrapper können eigene Container sein aber auch Module oder Libraries
