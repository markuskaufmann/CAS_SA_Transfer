...
* Wartung und Support: +
Bei einem on-premises-Hosting ist der Kunde selbst für die Wartung und Unterstützung der Infrastruktur verantwortlich. Dies kann ressourcenaufwändig sein und spezielles technisches Know-How erfordern.
* Eingeschränkte Skalierbarkeit: +
...
Die Möglichkeit, dass - je nach Kunde - die Alternative __A2) On-premises beim Kunden__ zum Einsatz kommen kann, wird nicht ausgeschlossen, sollte jedoch ein Ausnahmefall bleiben.
|**Begründung**
a|
...
* Deploymentprozess wäre besser darauf ausgerichtet, ein on-premises-Hosting beim Kunden zusätzlich zu unterstützen.
...
Da die Alternative __A2) On-premises beim Kunden__ des Entscheids __ADR-HST-10__ potenziell zum Einsatz kommen kann, wird es trotzdem möglich sein, eine einzelne Instanz für einen Kunden in der Cloud zu deployen.
|**Begründung**
a|
...
Zu beachten ist jedoch, dass die Alternative __A2) On-premises beim Kunden__ des Entscheids __ADR-HST-10__ potenziell zum Einsatz kommen kann, was in der Installation und dem Betrieb weiterer Physio Connect-Instanzen resultieren würde.
